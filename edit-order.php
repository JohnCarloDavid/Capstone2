...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "UPDATE tb_orders SET customer_name = ?, product_name = ?, quantity = ?, order_date = ?, status = ?, size = ? WHERE order_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssisssi', $customer_name, $product_name, $quantity, $order_date, $status, $size, $order_id);
